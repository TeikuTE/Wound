...
function AppendixG({ tweaks, basePage = 1 }) {
  ...
  return (<>
    <Spread id="appendix-g-opener" label="G · opener">
      ...
      <ChPage side="recto" showWatermark label="G · opener">
        <div className="el-appendix-opener">
          <div className="el-appendix-eyebrow">Appendix G</div>
          <h1 className="el-h1 el-appendix-opener__title">Temporal Pathologies</h1>
    ...
    <Spread id="appendix-g-01" label="TEMPORAL PATHOLOGIES · spread 1">
      <ChPage side="verso" runHead="Temporal Pathologies" pageNum={basePage + 0} label={`TEMPORAL PATHOLOGIES · p.${basePage + 0}`}>
        ...
        <div className="el-body"><p><b>{inline(`2. Probability Sight.`)}</b>{inline(` `)}<i>{inline(`You see Paradox as nausea. Unstable people, unstable rooms, unstable moments — they all produce the same low-grade vertigo. Useful. Exhausting. You will learn to leave rooms half a sentence before anyone else does.`)}</i></p></div>
        <div className="el-body"><p><b>{inline(`3. Asymmetric Memory.`)}</b>{inline(` `)}<i>{inline(`Half of any new memory imprints normally. The other half imprints to the opposite hemisphere of where it should go. You remember every face you've ever seen, but not which side of the face you saw first. Reading is fine. Writing is harder.`)}</i></p></div>
      ...
      <ChPage side="recto" runHead="Temporal Pathologies" pageNum={basePage + 1} label={`TEMPORAL PATHOLOGIES · p.${basePage + 1}`}>
        ...
        <div className="el-body"><p><b>{inline(`9. Loop-Echo Hearing.`)}</b>{inline(` `)}<i>{inline(`You hear your own footsteps a half-second after taking them. You hear other people's footsteps in real time. Most Echoes with Loop-Echo Hearing develop a slight limp because the body tries to walk on the rhythm of the echo and trips itself.`)}</i></p></div>
        <div className="el-body"><p><b>{inline(`10. Calibrated Forgetting.`)}</b>{inline(` `)}<i>{inline(`You forget specific things on a schedule. Wednesday mornings: the names of people you have not seen in a year. The first night of every season: the smell of the place you grew up. The forgetting is precise. It is not a malfunction. Something is doing it on purpose.`)}</i></p></div>
    ...
    <Spread id="appendix-g-02" label="TEMPORAL PATHOLOGIES · spread 2">
      <ChPage side="verso" runHead="Temporal Pathologies" pageNum={basePage + 2} label={`TEMPORAL PATHOLOGIES · p.${basePage + 2}`}>
        ...
        <div className="el-body"><p><b>{inline(`15. The Cold Spot.`)}</b>{inline(` `)}<i>{inline(`A specific volume of space, roughly two cubic feet, anywhere in your immediate environment, is six degrees colder than the rest of the room. You can feel where it is without looking. Other people feel it too but cannot locate it.`)}</i></p></div>
      ...
      <ChPage side="recto" runHead="Temporal Pathologies" pageNum={basePage + 3} label={`TEMPORAL PATHOLOGIES · p.${basePage + 3}`}>
        ...
        <div className="el-body"><p>{inline(`Optional — unlock after `)}<b>{inline(`3+ Memory losses`)}</b>{inline(` or `)}<b>{inline(`Paradox 8+`)}</b></p></div>
        <h3 className="el-section">{inline(`OPTIONAL COMPLICATIONS`)}</h3>
        <div className="el-body"><p>{inline(`Prolonged displacement does things to a mind that don't have names in linear medicine. The conditions below are optional. They are not assigned automatically — a player may choose to roll on this table when a Memory becomes Lost, or when Paradox reaches `)}<b>{inline(`8+`)}</b>{inline(`, if they want a more specific shape to their Echo's deterioration. Roll `)}<b>{inline(`d6`)}</b>{inline(`, or choose the condition that fits your Echo's current state.`)}</p></div>
        <hr className="el-section-rule" />
        <h4 className="el-subsection">{inline(`1 — Cotard's Delusion`)}</h4>
    ...
    <Spread id="appendix-g-03" label="TEMPORAL PATHOLOGIES · spread 3">
      <ChPage side="verso" runHead="Temporal Pathologies" pageNum={basePage + 4} label={`TEMPORAL PATHOLOGIES · p.${basePage + 4}`}>
        ...
        <hr className="el-section-rule" />
        <h4 className="el-subsection">{inline(`2 — Fregoli Delusion`)}</h4>
        <div className="el-body"><p>{inline(`Everyone you meet is the same person in a different disguise. The shopkeeper, the soldier, the woman on the train, the Faction Agent — all one entity, cycling through masks, watching you. You're not certain who it is. You're not certain it isn't you.`)}</p></div>
        <div className="el-body"><p><i>{inline(`Mechanical effect:`)}</i>{inline(` you cannot use the SYNC mode to build genuine trust with any new NPC — you can still manipulate, but not connect. Once per session, when an NPC acts against your interests, you may declare 'I knew it was you' — you gain `)}<b>{inline(`+1`)}</b>{inline(` on your next roll against them, but also gain `)}<b>{inline(`+1 Paradox`)}</b>{inline(` from the paranoid resonance.`)}</p></div>
        <div className="el-body"><p><i>{inline(`To remove:`)}</i>{inline(` form a genuine Recurring Thread with someone and protect them at personal cost.`)}</p></div>
        <hr className="el-section-rule" />
      ...
      <ChPage side="recto" runHead="Temporal Pathologies" pageNum={basePage + 5} label={`TEMPORAL PATHOLOGIES · p.${basePage + 5}`}>
        ...
        <hr className="el-section-rule" />
        <h4 className="el-subsection">{inline(`4 — The Glass Voice`)}</h4>
        <div className="el-body"><p>{inline(`When you speak about a specific period of your past — usually the period surrounding your most significant Loss — your voice takes on a metallic, ringing quality. You can hear it. Others can hear it. The Church finds it beautiful.`)}</p></div>
        <div className="el-body"><p><i>{inline(`Mechanical effect:`)}</i>{inline(` `)}<b>{inline(`-1`)}</b>{inline(` to SYNC rolls when the conversation touches on the affected period. However, once per session, you may lean into the Glass Voice deliberately — describe speaking about that period with raw honesty. The authenticity creates an involuntary resonance. Gain `)}<b>{inline(`+2`)}</b>{inline(` to one SYNC roll immediately following. Church Templars who hear the Glass Voice are momentarily distracted (`)}<b>{inline(`-1`)}</b>{inline(` to their Drive Off roll against you this encounter).`)}</p></div>
        <div className="el-body"><p><i>{inline(`To remove:`)}</i>{inline(` Lock a new Memory that postdates the loss.`)}</p></div>
        <hr className="el-section-rule" />
        <h4 className="el-subsection">{inline(`5 — Mirror Agnosia`)}</h4>
    ...
    <Spread id="appendix-g-04" label="TEMPORAL PATHOLOGIES · spread 4">
      <ChPage side="verso" runHead="Temporal Pathologies" pageNum={basePage + 6} label={`TEMPORAL PATHOLOGIES · p.${basePage + 6}`}>
        ...
        <hr className="el-section-rule" />
        <h4 className="el-subsection">{inline(`6 — Chronal Aphasia`)}</h4>
        <div className="el-body"><p>{inline(`Your internal sense of sequence has fractured. You know what happened but not when. Events from decades apart feel simultaneous. You lose track of which wounds are old and which are new.`)}</p></div>
...
AppendixG.pages = 8;
Object.assign(window, { AppendixG });
