  ...
  return (<>
    <Spread id="chen-5-opener" label={"MEMORY MANAGEMENT · opener"}>
      ...
      <ChPage side="recto" showWatermark label="MEMORY MANAGEMENT · opener">
        <div className="chapter-sigil"><Glyph name="rendition" size="sigil" stage={0} /></div>
    ...
    <Spread id="chen-5-02" label={"MEMORY MANAGEMENT · spread 2"}>
      ...
      <ChPage side="recto" runHead={"Memory Management"} pageNum={basePage + 3} label={`MEMORY MANAGEMENT · p.${basePage + 3}`}>
        <aside className="el-reflection"><p>{inline(`A Reed-Solomon code can reconstruct a corrupted message if enough of the parity blocks survive. Your Memories function the same way. As long as enough remain, the original message — You — can be reconstructed from the fragments. — EC`)}</p><p>{inline(`Nine slots. Five Active. Three Fading. One Locked. The structure reflects what your nervous system was already doing before you were Displaced. You never remembered everything you lived. You remembered what was worth the metabolic cost of remembering.`)}</p><p>{inline(`The Chronoverse imposes the same accounting on Echoes, except the cost is steeper and the negotiation is shorter. Without intervention, the cost wins. With intervention, the cost still wins, but more slowly, and you get to choose what pieces you lose first.`)}</p><p>{inline(`Your memories exist in four states. They will not stay where you put them.`)}</p><p>{inline(`ACTIVE MEMORIES (5 SLOTS)`)}</p><p>{inline(`Vivid, detailed, present in your mind. These provide +2 to matching actions. They're energetically expensive to maintain — high-amplitude waveforms with strong emotional mass. When you invoke an Active Memory for a bonus, you're literally harmonizing with your past self, using stored resonance to enhance your current action.`)}</p><p>{inline(`FADING MEMORIES (3 SLOTS)`)}</p><p>{inline(`Losing clarity. +1 to matching actions. One step from being Lost. The signal is still there, but noise is increasing — new memories broadcasting on similar frequencies, creating static that obscures the old ones. The amplitude is decreasing. Chen's model calls this entropy. She's half right. It's also interference.`)}</p><p>{inline(`LOCKED MEMORY (1 SLOT)`)}</p><p>{inline(`Permanently preserved. +3 to matching actions. Costs 3 Coherence to Lock. Cannot fade through normal means. You're spending tremendous energy to create a standing wave — a self-reinforcing pattern that resists entropy.`)}</p><p>{inline(`Locking is the most important choice you'll make. Choose a Memory that defines your core identity — something that, if lost, would mean you no longer recognize yourself. You can only Lock once.`)}</p><div className="el-sign">— EC</div></aside>
      </ChPage>
    </Spread>
    ...
    <Spread id="chen-5-04" label={"MEMORY MANAGEMENT · spread 4"}>
      <ChPage side="verso" runHead={"Memory Management"} pageNum={basePage + 6} footnotes={<FinderFootnotes notes={[{"n":22,"text":"This table is harder to read than the rest of the chapter. Like the book doesn't want me to look directly at it. Will note that and move on.","act":2,"late":false}]} />} label={`MEMORY MANAGEMENT · p.${basePage + 6}`}>
        <div className="el-body"><p>{inline(`When you gain a Mutation — from Paradox Breach, from a Faction trade, from a Crisis outcome that specifies it — choose one from this list, or create your own in the same register. These are not personality quirks. They are the Chronoverse writing itself onto your body.`)}<FootnoteMarker n={22} act={2} /></p><span className="gly-inline-row"><Glyph name="drift" size="inline" stage={0} /></span></div>
    ...
    <Spread id="chen-5-05" label={"MEMORY MANAGEMENT · spread 5"}>
      <ChPage side="verso" runHead={"Memory Management"} pageNum={basePage + 8} footnotes={<FinderFootnotes notes={[{"n":23,"text":"Started noticing little circle marks in Chen's margins next to certain mechanical terms. They look like physics notation. There's a key in Appendix A. Saving it for later.","act":2,"late":false}]} />} label={`MEMORY MANAGEMENT · p.${basePage + 8}`}>
        <div className="el-body"><p><b>{inline(`12+`)}</b>{inline(` Against all odds, it returns. The Memory enters an Active slot, pushing all others down — this may force another Memory to become Fading. Gain +1 Paradox from the strain on causality.`)}<FootnoteMarker n={23} act={2} /></p><span className="gly-inline-row"><Glyph name="ripple" size="inline" stage={0} /></span></div>
        <table className="tbl-chen tbl-chen--oracle"><tbody><tr><td className="num">{`10-11`}</td><td className="oracle-name">{``}</td><td className="oracle-desc">{inline(`A fragment returns. The Memory comes back as Fading. You remember the shape of it, but not the texture. Gain +1 Paradox.`)}</td></tr><tr><td className="num">{`7-9`}</td><td className="oracle-name">{``}</td><td className="oracle-desc">{inline(`The effort fails. Lose 1 additional Coherence. The Memory is gone. This attempt is the last time you'll look for it.`)}</td></tr></tbody></table>
        <div className="el-body"><p><b>{inline(`6-`)}</b>{inline(` The void stares back. You recover a False Memory — a comforting lie that fills the gap. The Oracle decides what it is. You believe it completely. Gain a Scar: Beautiful Lie.`)}</p></div>
      ...
      <ChPage side="recto" runHead={"Memory Management"} pageNum={basePage + 9} footnotes={<FinderFootnotes notes={[{"n":28,"text":"The Holding Action is the most honest mechanic in the book. You can refuse the consequence once. Once. Then the bill comes due. I have used it twice this week in real life.","act":3,"late":false}]} />} label={`MEMORY MANAGEMENT · p.${basePage + 9}`}>
        <h4 className="el-subsection">{inline(`CHRONICLE BONUSES`)}</h4>
        <div className="el-body"><p>{inline(`Detailed Entries grant +1 Coherence once per session, when your entry: describes a scene in vivid detail, reflects on your emotional state, connects to your Anchor or Memories, and advances your understanding of your condition.`)}<FootnoteMarker n={28} act={3} /></p><span className="gly-inline-row"><Glyph name="burden" size="inline" stage={0} /></span></div>
    ...
    <Spread id="chen-5-06" label={"MEMORY MANAGEMENT · spread 6"}>
      <ChPage side="verso" runHead={"Memory Management"} pageNum={basePage + 10} footnotes={<FinderFootnotes notes={[{"n":24,"text":"This page is in Chen's voice but it reads like a confession. She did everything right and lost everything anyway. The Chronicle is becoming more than a journal. I am writing things in here I have never told anyone.","act":2,"late":false}]} />} label={`MEMORY MANAGEMENT · p.${basePage + 10}`}>
        <div className="el-body"><p>{inline(`I followed every procedure. Every rule. Every preservation protocol in Chen's manual. I maintained Coherence above +5 for two centuries. I Locked my defining Memory. I reinforced my Anchor quarterly. I wrote in the Chronicle every single day.`)}<FootnoteMarker n={24} act={2} /></p><span className="gly-inline-row"><Glyph name="null" size="inline" stage={0} /></span></div>
        ...
        <aside className="el-reflection el-reflection--theory"><p>{inline(`The holographic principle holds that the information content of a volume of space can be encoded on its boundary. Your Chronicle is your boundary. What you write down is the surface from which your interior — your continuous self — can be reconstructed. If the volume is lost but the boundary remains, you can be rebuilt. — EC`)}</p><div className="el-sign">— EC</div></aside>
