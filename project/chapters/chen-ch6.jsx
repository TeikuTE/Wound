...
function ChenCh6({ tweaks, basePage = 1 }) {
  ...
  return (<>
    ...
    <Spread id="chen-6-02" label={"CRISIS MANAGEMENT · spread 2"}>
      <ChPage side="verso" runHead={"Crisis Management"} pageNum={basePage + 2} footnotes={<FinderFootnotes notes={[{"n":25,"text":"Did not open this book today. Found it open on my desk. The ink in the margins on this page is a different color — a deep, bruised purple. Still wet to the touch.","act":3,"late":false}]} />} label={`CRISIS MANAGEMENT · p.${basePage + 2}`}>
    ...
    <Spread id="chen-6-03" label={"CRISIS MANAGEMENT · spread 3"}>
      <ChPage side="verso" runHead={"Crisis Management"} pageNum={basePage + 4} label={`CRISIS MANAGEMENT · p.${basePage + 4}`}>
        ...
        <table className="tbl-chen tbl-chen--oracle"><tbody><tr><td className="num">{`10-11`}</td><td className="oracle-name">{`Partial Stabilization. The Crisis ends, but you are permanently changed. Anchor Strength restored to 1. One of your Origin Details is permanently lost`}</td><td className="oracle-desc">{inline(`you remember having a daughter, but not her face. You remember the lab, but not what you were trying to prove.`)}</td></tr></tbody></table>
      </ChPage>
      <ChPage side="recto" runHead={"Crisis Management"} pageNum={basePage + 5} label={`CRISIS MANAGEMENT · p.${basePage + 5}`}>
        <table className="tbl-chen tbl-chen--oracle"><tbody><tr><td className="num">{`7-9`}</td><td className="oracle-name">{``}</td><td className="oracle-desc">{inline(`Barely Survive. You stabilize, but the Anchor is gone. Anchor Strength remains at 0. You can no longer gain Coherence from Chronicling or Anchor Reinforcement. You are untethered. This is how Alan started.`)}</td></tr></tbody></table>
        ...
        <table className="tbl-chen tbl-chen--oracle"><tbody><tr><td className="num">{`7-9`}</td><td className="oracle-name">{``}</td><td className="oracle-desc">{inline(`Escape but wounded. Lose 3 Coherence OR mark all Stress on one Asset.`)}</td></tr></tbody></table>
    ...
    <Spread id="chen-6-04" label={"CRISIS MANAGEMENT · spread 4"}>
      <ChPage side="verso" runHead={"Crisis Management"} pageNum={basePage + 6} label={`CRISIS MANAGEMENT · p.${basePage + 6}`}>
        ...
        <table className="tbl-chen tbl-chen--oracle"><tbody><tr><td className="num">{`7-9`}</td><td className="oracle-name">{``}</td><td className="oracle-desc">{inline(`Trapped for `)}<b>{inline(`1d6`)}</b>{inline(` iterations. Each iteration, one Active Memory becomes Fading. Then escape.`)}</td></tr></tbody></table>
        ...
        <table className="tbl-chen tbl-chen--oracle"><tbody><tr><td className="num">{`7-9`}</td><td className="oracle-name">{``}</td><td className="oracle-desc">{inline(`Escape but contaminated. Gain +2 Paradox, suffer -1 Coherence from exposure.`)}</td></tr></tbody></table>
      </ChPage>
